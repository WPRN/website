<template>
  ...
  >
    ...
    <v-card-subtitle
      ...
      v-html="'WPRN-' + $options.filters.highlight(project.pubId, filters.search.split(' '))"
    ></v-card-subtitle>
    <v-card-text
      ...
      <v-row>
          ...
          <template v-if="project.thematics.length > 10&&!showThematics">
            <template v-for="(thematic, index) in project.thematics">
              <v-chip
                ...
                :color="!pageMode&&filters.thematics.includes(thematics)?'white':''"
              >{{thematic}}</v-chip>
              <span
                :key="index"
                v-if="index===11"
        ...
        <v-col cols="12" md="6" xl="4" class="subtitle-1">
          ...
          <v-chip
            ...
            :color="!pageMode&&filters.zone.includes(zone)?'#c4c4c4':'accent'"
            :key="index"
            label
            light
        ...
        <v-col cols="12" class="subtitle-1">
          ...
          <template v-if="pageMode">
            <span
              v-html="$options.filters.highlight(project.contact_lastname, filters.search.split(' '))"
            ></span>
          ...
          <template v-else>
            ...
            ></span>
            {{', ' + project.contact_firstname}}
            <template v-if="project.contact_entity">
              <template v-if="!pageMode">
                <template v-if="filters.search&&filters.search.length">
                  <span
                    v-html="'(' + $options.filters.highlight(project.contact_entity, filters.search.split(' ')) + ')'"
                  ></span>
                </template>
        ...
        <v-col cols="12" class="subtitle-1 pr-12">
          ...
          <template v-else>
            <template v-if="project.description.length>400">
              <template v-if="filters.search&&filters.search.length">
                <p
                  v-html="$options.filters.nl2br($options.filters.highlight($options.filters.truncate(project.description, 400, '(read more)', '/item/'+ project.pubId), filters.search.split(' ')))"
                ></p>
              </template>
              <template v-else>
                <p
                  v-html="$options.filters.nl2br($options.filters.truncate(project.description, 400, '(read more)', '/item/'+ project.pubId))"
                ></p>
              </template>
            </template>
            <template v-else>
              <p
                v-html="$options.filters.nl2br($options.filters.highlight(project.description, filters.search.split(' ')))"
              ></p>
            </template>
          </template>
        </v-col>
    ...
    return {
      ...
      socialChannels: [
        ...
        },
        {
          ...
          icon: "email",
          url:
            `mailto:?bcc=contact@wprn.org&subject=WPRN Project&body=Hello,%0d%0a%0d%0aPlease check this project called ` +
            this.project.name +
